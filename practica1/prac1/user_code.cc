...
//0,0,0, 0.5
void dibuja_cubo(float posX, float posY, float posZ, float altura ){
...
void draw_triangulos(vector<double> &Vertices, vector<unsigned long> &Caras, int color, bool mitad){
    ...
    //draw_lineas(Vertices, Caras, mitad);
...
//**************************************************************************
void draw_ajedrez(vector<double> &Vertices, vector<unsigned long> &Caras, int color, bool mitad){
...
/*
        //Dibujamos cada triángulo de un color distinto para veros con más claridad.
        if(i%2==0) glColor3f(0.5,0,0.1);
        else glColor3f(0,1,0);
